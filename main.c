...
int main() {
     ...
     * Each function's signature is stated in the utils.h file, with the definitions started in utils.c. You 
don't
     ...
     *
     * However, if you come up with a clever solution to solve a problem or implement a solution, we'd love to 
see it.
     *
     * If you get stuck with a C concept or are looking for a pointer, head over to Piazza and feel free to post 
your code.
     * Instructors will be happy to point you in the right direction and students are encouraged to also provide 
support.
     *
     * Now: 3, 2, 1... Begin!
     */
    ...
    // Next, define and call your parseInput function passing the string input and pkt as arguments
    ...
    // We need to create a new buffer in memory (seperate from the input buffer) to hold our packet. 
    // Since we only know the size of our packet since parsing it, we will have to create this memory at runtime. 
    // You'll have to use malloc (https://man7.org/linux/man-pages/man3/malloc.3.html)
    // to create a pointer to a new buffer of the correct size (hdrLen + dataLen)
     uint8_t* memory = malloc(packet->hdrLen + packet->dataLen);
    ...
    */
    ...
    // and call it here
     printPacket(memory, packet->hdrLen + packet->dataLen);
